...
bool ps2_output_buffer_full(void){
    return ps2_status().output_buffer_full;
}

bool ps2_input_buffer_full(void){
    return ps2_status().input_buffer_full;
}

bool ps2_system_flag(void){
    return ps2_status().system_flag;
}

bool ps2_command_data(void){
    return ps2_status().command_data;
}

bool ps2_timeout_error(void){
    return ps2_status().timeout_error;
}

bool ps2_parity_error(void){
    return ps2_status().parity_error;
}

bool ps2_zero(void){
    return ps2_status().zero;
}
